...
def Phylanx(__phylanx_arg=None, **kwargs):
    class _PhylanxDecorator(object):
        def __init__(self, fn):
            ...
            """
            valid_kwargs = [
                'debug', 'target', 'compiler_state', 'performance',
                'localities'
            ]

            self.backends_map = {'PhySL': Phylanx, 'OpenSCoP': OpenSCoP}
            self.backend = self.get_backend(kwargs.get('target'))
            for key in kwargs.keys():
                if key not in valid_kwargs:
                    raise NotImplementedError(
                        "Unknown Phylanx argument '%s'." % key)

            # Obtain global environment if the object is a function.
            if inspect.isclass(fn):
                kwargs['fglobals'] = None
            else:
                kwargs['fglobals'] = fn.__globals__

            python_src = self.get_python_src(fn)
            python_ast = self.get_python_ast(python_src, fn)

            self.kwargs = kwargs
            self.backend = Phylanx(fn, python_ast,
                                                           kwargs)
        ...
        def compile_function(self, func):
            ...
            if func.__name__ == '<lambda>':
                ...
                fn_body = ast.FunctionDef(name=func.__name__,
            ...
            return Phylanx(func, fn_ast, self.kwargs)
